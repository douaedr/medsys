...
import {
  Users, Calendar, Stethoscope, FileText, BarChart3, Download,
...
export default function DirecteurDashboard() {
  ...
  const titles = {
    ...
    patients: 'Patients', medecins: 'Médecins', rapports: 'Rapports',
  }
  ...
  return (
    <DashboardLayout title={titles[tab]} subtitle="Direction — MedSys">
          ...
          <div className="card p-6">
            ...
            {patients.slice(0,5).map(p => (
              <div key={p.id} className="flex items-center gap-3 py-2 border-b border-slate-100 last:border-0 cursor-pointer hover:bg-slate-50 rounded-lg px-2" onClick={() => handleViewDossier(p)}>
                ...
                <div className="flex-1"><div className="font-semibold text-slate-900 text-sm">{p.prenom} {p.nom}</div>
                  <div className="text-xs text-slate-500">{p.email || '—'}</div></div>
              </div>
            ))}
          </div>
          <div className="card p-6">
            ...
            <div className="space-y-2">
              <button onClick={() => setTab('patients')} className="w-full text-left p-3 rounded-lg hover:bg-slate-50 flex items-center gap-3">
                <Users className="w-5 h-5 text-primary-600" /><div><div className="font-semibold text-sm">Voir les patients</div><div className="text-xs text-slate-500">{patients.length} enregistrés</div></div></button>
              <button onClick={() => setTab('medecins')} className="w-full text-left p-3 rounded-lg hover:bg-slate-50 flex items-center gap-3">
                <Stethoscope className="w-5 h-5 text-primary-600" /><div><div className="font-semibold text-sm">Voir les médecins</div><div className="text-xs text-slate-500">{medecins.length} médecins</div></div></button>
              <button onClick={() => setTab('rapports')} className="w-full text-left p-3 rounded-lg hover:bg-slate-50 flex items-center gap-3">
                <FileText className="w-5 h-5 text-primary-600" /><div><div className="font-semibold text-sm">Rapports PDF</div><div className="text-xs text-slate-500">Générer des rapports</div></div></button>
            </div>
          </div>
        </div>
      </>)}
              ...
              <div className="space-y-2">
                {rdv.slice(0,10).map((r,i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                    <div><div className="text-sm font-semibold">{r.medecinNom || r.doctorName || 'Médecin'}</div>
                      <div className="text-xs text-slate-500">{formatDateTime(r.date)} {r.heure ? `à ${r.heure}` : ''}</div></div>
      ...
      {tab === 'patients' && (
        <div className="card">
          <div className="p-6 border-b border-slate-100 flex items-center justify-between gap-4 flex-wrap">
            <div><h3 className="font-bold text-slate-900">Liste des patients</h3>
              <p className="text-sm text-slate-500 mt-0.5">{patients.length} patient(s)</p></div>
          ...
          {patients.length === 0 ? <EmptyState icon={Users} title="Aucun patient" description="Aucun résultat." /> : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50"><tr className="text-left text-xs font-semibold text-slate-500 uppercase">
                  <th className="px-6 py-3">Patient</th><th className="px-6 py-3">Date naissance</th>
                  <th className="px-6 py-3">Email</th><th className="px-6 py-3 text-right">Actions</th>
                </tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {patients.map(p => (
                    <tr key={p.id} className="hover:bg-slate-50">
                      <td className="px-6 py-4"><div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary-500 to-accent-600 flex items-center justify-center text-white text-xs font-bold">{(p.prenom?.[0]||'')+(p.nom?.[0]||'')}</div>
                        <div><div className="font-semibold text-sm">{p.prenom} {p.nom}</div><div className="text-xs text-slate-500">CIN: {p.cin||'—'}</div></div>
                      </div></td>
        ...
        <div className="card">
          ...
          {medecins.length === 0 ? <EmptyState icon={Stethoscope} title="Aucun médecin" description="Aucun médecin enregistré." /> : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50"><tr className="text-left text-xs font-semibold text-slate-500 uppercase">
                  <th className="px-6 py-3">Médecin</th><th className="px-6 py-3">Spécialité</th>
                  <th className="px-6 py-3">Service</th><th className="px-6 py-3">Matricule</th>
                </tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {medecins.map(m => (
      ...
      {tab === 'rapports' && (
        <div className="space-y-6">
          <div className="card p-6">
            <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2"><FileText className="w-5 h-5 text-primary-600" /> Rapports disponibles</h3>
            <div className="grid md:grid-cols-2 gap-4">
              {patients.slice(0,10).map(p => (
                <div key={p.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary-500 to-accent-600 flex items-center justify-center text-white text-xs font-bold">{(p.prenom?.[0]||'')+(p.nom?.[0]||'')}</div>
                    <div><div className="font-semibold text-sm">{p.prenom} {p.nom}</div><div className="text-xs text-slate-500">ID: {p.id}</div></div>
                  </div>
                  <button onClick={() => handleExportPatientPdf(p.id)} disabled={generatingPdf}
                    className="btn-outline btn-sm"><Download className="w-4 h-4" /> PDF</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => { setSelectedPatient(null); setDossier(null) }}>
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white z-10">
              <div><h3 className="font-bold text-lg">{selectedPatient.prenom} {selectedPatient.nom}</h3>
                <p className="text-sm text-slate-500">CIN: {selectedPatient.cin||'—'} · ID #{selectedPatient.id}</p></div>
            ...
            <div className="p-6 space-y-6">
              ...
              {dossier?.antecedents?.length > 0 && (<div>
                <h4 className="font-semibold text-sm uppercase tracking-wide text-slate-500 mb-3">Antécédents</h4>
                <ul className="space-y-2 text-sm">{dossier.antecedents.map((a,i) => (
                  <li key={i} className="p-3 bg-slate-50 rounded-lg"><div className="font-semibold">{a.typeAntecedent||a.type}</div><div className="text-slate-600">{a.description}</div></li>
                ))}</ul>
              </div>)}
              {dossier?.consultations?.length > 0 && (<div>
                ...
                <ul className="space-y-2 text-sm">{dossier.consultations.slice(0,5).map((c,i) => (
                  <li key={i} className="p-3 bg-slate-50 rounded-lg"><div className="font-semibold">{formatDateTime(c.dateConsultation||c.date)}</div><div className="text-slate-600">{c.motif||c.diagnostic||'Consultation'}</div></li>
                ))}</ul>
              </div>)}
            </div>
          </div>
